...
export default function TokenizationPage() {
  ...
  return (
    <main style={{ padding: '32px 24px', maxWidth: 720, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 14, marginBottom: 28 }}>
        {INFO_CARDS.map((c) => (
          <div key={c.title} style={{ ...card, display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <p style={{ fontSize: 12, color: '#64748B', margin: 0, lineHeight: 1.5 }}>{c.desc}</p>
          </div>
        ))}
      </div>
        ...
        {loading ? (
          ...
          <form onSubmit={handleSave}>
            <div style={{ marginBottom: 16 }}>
              ...
              />
              <p style={{ fontSize: 11, color: '#CBD5E1', margin: '5px 0 0' }}>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <button
                ...
                disabled={saving || !wallet}
                style={{
                  background: saving || !wallet ? '#E2E8F0' : 'linear-gradient(135deg, #D97706, #3B5BFA)',
                  color: saving || !wallet ? '#64748B' : '#0d0e11',
      ...
      {/* Wallet info */}
      {wallet && (
        <p style={{ fontSize: 12, fontFamily: 'monospace', color: '#94A3B8' }}>
