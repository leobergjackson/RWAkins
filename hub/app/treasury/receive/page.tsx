...
function CopyBtn({ text }: { text: string }) {
  ...
  return (
    <button onClick={copy} style={{ padding:'4px 10px', borderRadius:6, border:`1px solid ${copied ? TEAL : BDR}`, background:copied?'rgba(0,229,204,0.08)':'transparent', color:copied?TEAL:'rgba(255,255,255,0.4)', fontSize:10, cursor:'pointer' }}>
      {copied ? '✓ Copied' : '⎘ Copy'}
    </button>
  )
}
    ...
    <img
      ...
      onError={(e) => {
        ...
        if (parent) {
          ...
          fb.style.cssText = 'width:140px;height:140px;border-radius:10px;border:1px solid rgba(0,229,204,0.2);display:flex;align-items:center;justify-content:center;font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:10px;box-sizing:border-box'
  ...
  return (
    <div style={{ background: BG, minHeight: '100vh', padding: '24px', color: '#fff', fontFamily: '"Inter",system-ui,sans-serif' }}>
      ...
      <div style={{ background: CARD, border: `1px solid ${BDR}`, borderRadius: 14, padding: '20px 24px', marginBottom: 20 }}>
        ...
        {loading ? (
          <div style={{ color:'rgba(255,255,255,0.3)', fontSize:12 }}>Loading wallets...</div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {wallets.map(w => (
              <div key={w.id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 16px', background:'rgba(255,255,255,0.02)', borderRadius:10, border:`1px solid ${BDR}`, flexWrap:'wrap', gap:12 }}>
                <div style={{ display:'flex', alignItems:'center', gap:14 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontFamily:MONO, fontSize:11, color:'rgba(255,255,255,0.5)' }}>{w.address}</div>
                    <div style={{ fontSize:11, color:'rgba(255,255,255,0.35)', marginTop:2 }}>{w.balance.toLocaleString()} {w.symbol} · ${w.usdValue.toLocaleString()}</div>
                  </div>
                </div>
        ...
        <div style={{ background: CARD, border: `1px solid ${BDR}`, borderRadius: 14, padding: '22px' }}>
          ...
          <form onSubmit={handleCreate}>
            {[
              ...
              <div key={f.key} style={{ marginBottom: 14 }}>
                ...
                <input type={f.type} placeholder={f.placeholder} value={(form as any)[f.key]} onChange={upd(f.key)} required={f.label.includes('*')}
                  style={{ width:'100%', padding:'10px 12px', borderRadius:8, border:`1px solid ${BDR}`, background:'rgba(255,255,255,0.04)', color:'#fff', fontSize:13, outline:'none', boxSizing:'border-box' }} />
              </div>
            ))}
              ...
              {[
                ...
                <div key={f.key}>
                  ...
                  <select value={(form as any)[f.key]} onChange={upd(f.key)}
                    style={{ width:'100%', padding:'10px 12px', borderRadius:8, border:`1px solid ${BDR}`, background:'#0e0e1a', color:'#fff', fontSize:13, outline:'none' }}>
            ...
            <div style={{ marginBottom:18 }}>
              <label style={{ fontSize:11, color:'rgba(255,255,255,0.5)', display:'block', marginBottom:6 }}>EXPIRY DATE</label>
        ...
        <div>
            ...
            <div style={{ background:'rgba(0,229,204,0.04)', border:`1px solid rgba(0,229,204,0.25)`, borderRadius:14, padding:'22px', marginBottom:16 }}>
              ...
              <div style={{ display:'flex', gap:16, alignItems:'flex-start', marginBottom:14, flexWrap:'wrap' }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:12, marginBottom:6 }}>
                    <span style={{ color:'rgba(255,255,255,0.4)' }}>Network: </span>{created.network}
                  </div>
                  <div style={{ fontSize:12, marginBottom:6 }}>
                    <span style={{ color:'rgba(255,255,255,0.4)' }}>Expires: </span>{created.expiresAt}
                  </div>
                  {created.description && (
                    <div style={{ fontSize:12, marginBottom:6 }}>
                      <span style={{ color:'rgba(255,255,255,0.4)' }}>Note: </span>{created.description}
                    </div>
                  )}
                </div>
              </div>
              <div style={{ fontSize:10, color:'rgba(255,255,255,0.35)', marginBottom:8 }}>Share link:</div>
              <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                <div style={{ flex:1, fontFamily:MONO, fontSize:10, color:'rgba(255,255,255,0.5)', wordBreak:'break-all', padding:'8px', background:'rgba(255,255,255,0.03)', borderRadius:6 }}>{created.shareLink}</div>
          ...
          <div style={{ background:CARD, border:`1px solid ${BDR}`, borderRadius:14, padding:'22px' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              {requests.map(r => (
                <div key={r.id} style={{ padding:'12px 14px', background:'rgba(255,255,255,0.02)', borderRadius:10, border:`1px solid ${BDR}` }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:6 }}>
                    <div>
                      <span style={{ fontFamily:MONO, fontWeight:700, fontSize:13, color:statusColor(r.status) }}>{r.amount} {r.asset}</span>
                      <span style={{ fontSize:10, color:'rgba(255,255,255,0.35)', marginLeft:8 }}>{r.network}</span>
                    </div>
                    <span style={{ fontSize:10, padding:'2px 8px', borderRadius:20, background:`${statusColor(r.status)}18`, color:statusColor(r.status) }}>
                      {r.status}
                    </span>
                  </div>
                  {r.description && <div style={{ fontSize:11, color:'rgba(255,255,255,0.45)', marginBottom:4 }}>{r.description}</div>}
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    <div style={{ fontSize:10, color:'rgba(255,255,255,0.25)' }}>Created {r.createdAt} · Exp {r.expiresAt}</div>
                    <CopyBtn text={r.shareLink} />
                  </div>
                </div>
              ))}
              {requests.length === 0 && <div style={{ color:'rgba(255,255,255,0.3)', fontSize:12, textAlign:'center', padding:'16px 0' }}>No payment requests yet</div>}
            </div>
          </div>
        </div>
      </div>
