...
import InvoiceLayout from '@/app/invoice/layout';
import DashboardPage from '@/app/dashboard/page';
import { FileText, LayoutDashboard, Wrench } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-white overflow-hidden font-sans">
      ...
      <nav className="flex items-center gap-2 p-3 bg-slate-900 border-b border-slate-800 shrink-0">
        ...
        <a 
          href="#invoice" 
          onClick={(e) => { e.preventDefault(); window.location.hash = 'invoice'; }}
          className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold transition-colors ${activeTab === 'invoice' ? 'bg-indigo-500/20 text-indigo-400' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'}`}
        >
      ...
      <div className="flex-1 overflow-y-auto bg-slate-50 relative">
        ...
        <div style={{ display: activeTab === 'invoice' ? 'block' : 'none', minHeight: '100%' }}>
           <InvoiceLayout>
             <InvoicePage />
           </InvoiceLayout>
        </div>
        <div style={{ display: activeTab === 'tools' ? 'block' : 'none' }} className="p-8">
           ...
           <p className="text-slate-500 text-sm">More tools (Credit Passport, Family Vault, AI Lending) coming soon to the extension side panel...</p>
        </div>
      </div>
    </div>
  );
