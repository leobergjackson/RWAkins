              ...
              <tr key={m.asset} style={{ borderBottom: `1px solid ${BORDER}` }}>
                ...
                <td style={td}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <div style={{ flex: 1, height: 6, background: '#F8FAFC', borderRadius: 3, overflow: 'hidden', maxWidth: 100 }}>
      ...
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, padding: 20 }}>
        ...
        {[
          ...
          <div key={t.tier} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: `1px solid ${BORDER}`, fontSize: 12 }}>
            ...
            <span style={{ color: '#0A0F2E' }}>{t.desc}</span>
          </div>
        ))}
      </div>
    </div>
...
function Stat({ label, v }: { label: string; v: string }) {
  return (
    <div>
      ...
      <div style={{ fontSize: 18, fontWeight: 700, color: '#0A0F2E', marginTop: 4, fontFamily: MONO }}>{v}</div>
    </div>
  )
}
