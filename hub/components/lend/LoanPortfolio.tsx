...
export default function LoanPortfolio({ isConnected = false }: { isConnected?: boolean }) {
  ...
  return (
    <div style={{ padding: 28, display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'inline-flex', background: '#ffffff', border: `1px solid ${BORDER}`, borderRadius: 10, padding: 4, alignSelf: 'flex-start' }}>
              ...
              <tbody>
                {FALLBACK_LOANS.map(l => (
                  <>
                    ...
                    {expanded === l.id && (
                      <tr key={`${l.id}-d`}>
                        <td colSpan={7} style={{ padding: '14px 16px', background: '#F8FAFC' }}>
            ...
            {FALLBACK_MY_POSITIONS.collateral.map(c => (
              <div key={c.asset} style={{ marginBottom: 10 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}>
                  <span style={{ color: '#0A0F2E' }}>{c.asset}: {c.amount} (${c.value.toLocaleString()})</span>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td colSpan={2} style={{ ...td, textAlign: 'right' }}>
                  <button onClick={handleClaimAll} style={gate({ padding: '6px 14px', borderRadius: 6, background: A, color: '#0A0F2E', border: 'none', fontSize: 12, fontWeight: 600, cursor: 'pointer' })}>Claim All →</button>
                </td>
              </tr>
            </tbody>
          </table>
...
function Det({ label, v }: { label: string; v: string }) {
  return (
    <div>
      ...
      <div style={{ fontSize: 12, color: '#0A0F2E', fontFamily: MONO }}>{v}</div>
    </div>
  )
}
...
const th: React.CSSProperties = {
  ...
  color: MUTED2, background: '#F8FAFC',
}
const td: React.CSSProperties = { padding: '12px 16px', color: '#0A0F2E' }
